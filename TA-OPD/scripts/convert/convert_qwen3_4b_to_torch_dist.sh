...
# Convert Qwen3-4B HuggingFace checkpoint to slime/Megatron torch_dist format.
#
# Usage on the server:
#   cd /path/to/slime-main
#   bash scripts/convert/convert_qwen3_4b_to_torch_dist.sh
#
# Optional overrides:
...
HF_MODEL="${HF_MODEL:-/path/to/models/Qwen3/4B}"
SAVE_DIR="${SAVE_DIR:-/path/to/models/Qwen3/4B_torch_dist}"

# Qwen3-4B is small enough to convert on one GPU. Increase only if needed.
...
echo "=== Qwen3-4B HF -> torch_dist conversion ==="
