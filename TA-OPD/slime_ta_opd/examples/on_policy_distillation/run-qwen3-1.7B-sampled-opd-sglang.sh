...
#   student rollout/inference: Qwen3-1.7B SGLang engine on one GPU
#   student training: Qwen3-1.7B Megatron on two GPUs
#
# Usage:
...
FALLBACK_SITE_PACKAGES="${FALLBACK_SITE_PACKAGES:-/path/to/conda/envs/opsd/lib/python3.10/site-packages}"

TEACHER_MODEL="${TEACHER_MODEL:-/path/to/models/Qwen3/4B}"
STUDENT_HF="${STUDENT_HF:-/path/to/models/Qwen3/1.7B/Qwen_Qwen3-1.7B}"
STUDENT_TORCH_DIST="${STUDENT_TORCH_DIST:-/path/to/models/Qwen3/1.7B/Qwen_Qwen3-1.7B_torch_dist}"

DATA_DIR="${DATA_DIR:-/path/to/storage/user/OPSD_OnPolicyDistillation-main/data/grpo_processed}"
PROMPT_DATA="${PROMPT_DATA:-${DATA_DIR}/train.parquet}"

RUN_NAME="${RUN_NAME:-qwen3_1_7b_student_qwen3_4b_teacher_sampled_opd_$(date +%Y%m%d_%H%M%S)}"
...
OPD_TOKEN_BANK_PAIR_ID="${OPD_TOKEN_BANK_PAIR_ID:-qwen3_4b_to_qwen3_1p7b}"
...
OPD_BUDGET_MASK_SEED="${OPD_BUDGET_MASK_SEED:-42}"
...
cleanup() {
  ...
  if [[ "${ALLOW_GLOBAL_RAY_STOP}" == "1" ]]; then
    ray stop --force >/dev/null 2>&1
  else
    pkill -f "${RAY_TEMP_DIR}" >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
...
TORCH_CUDA_RUNTIME_LIB="$(python3 -c 'from pathlib import Path; import torch; site_packages = Path(torch.__file__).resolve().parents[1]; print(site_packages / "nvidia" / "cuda_runtime" / "lib")')"
export LD_LIBRARY_PATH="${TORCH_CUDA_RUNTIME_LIB}:${LD_LIBRARY_PATH:-}"
...
if [[ "${COLOCATE}" == "1" ]]; then
  REQUIRED_RAY_GPUS="${ACTOR_NUM_GPUS_PER_NODE}"
else
...
echo "ALLOW_GLOBAL_RAY_STOP = ${ALLOW_GLOBAL_RAY_STOP}"
...
echo "OPD_MASK_SEED      = ${OPD_BUDGET_MASK_SEED}"
...
if [[ "${OPD_TOPK_METRICS_K}" != "0" || -n "${OPD_TOKEN_BANK_DIR}" || "${OPD_BUDGET_MASK}" != "none" ]]; then
  ...
  TIP_COMPAT_ARGS+=(
    ...
    --opd-budget-mask-seed "${OPD_BUDGET_MASK_SEED}"
...
CUDA_VISIBLE_DEVICES="${RAY_GPUS}" ray job submit \
  ...
  --no-wait \
  --runtime-env-json="{\"env_vars\":{\"PYTHONPATH\":\"${PYTHONPATH}\",\"LD_LIBRARY_PATH\":\"${LD_LIBRARY_PATH}\",\"CUDA_DEVICE_MAX_CONNECTIONS\":\"${CUDA_DEVICE_MAX_CONNECTIONS}\",\"NCCL_CUMEM_ENABLE\":\"${NCCL_CUMEM_ENABLE}\"}}" \
  -- python3 train.py \
  --actor-num-nodes 1 \
...
while true; do
  ...
  if [[ -z "${JOB_STATUS}" ]]; then
    if printf '%s\n' "${STATUS_OUTPUT}" | grep -qi "Job .* succeeded"; then
      JOB_STATUS="SUCCEEDED"
    elif printf '%s\n' "${STATUS_OUTPUT}" | grep -qi "Job .* failed"; then
      JOB_STATUS="FAILED"
    elif printf '%s\n' "${STATUS_OUTPUT}" | grep -qi "Job .* stopped"; then
      JOB_STATUS="STOPPED"
    fi
  fi
  case "${JOB_STATUS}" in
