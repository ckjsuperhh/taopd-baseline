...
DIAG_DIR="${DIAG_DIR:-${STORY_ROOT}/14b_to_1p7b_diag300}"
DRIVER_LOG="${LOG_DIR}/${RUN_NAME}_driver.log"

mkdir -p "${LOG_DIR}" "${DIAG_DIR}"
cd "${SLIME_DIR}"

echo "=== 14B -> 1.7B 300-context fixed-context diagnostic ==="
...
#   GPU0: 14B teacher SGLang during training
...
ROLLOUT_NUM_GPUS=1 \
TEACHER_PORT=13271 \
TEACHER_NCCL_PORT=23271 \
RAY_PORT=26471 \
RAY_DASHBOARD_PORT=8371 \
RAY_OBJECT_MANAGER_PORT=28671 \
RAY_NODE_MANAGER_PORT=28672 \
RAY_DASHBOARD_AGENT_LISTEN_PORT=28673 \
RAY_DASHBOARD_AGENT_GRPC_PORT=28674 \
RAY_METRICS_EXPORT_PORT=28675 \
RAY_TEMP_DIR=/tmp/slime_ray_26471 \
...
OPD_TOKEN_BANK_PAIR_ID=qwen3_14b_to_qwen3_1p7b \
OPD_EXACT_CMASS=1 \
OPD_EXACT_CMASS_MAX_UNION=128 \
SAVE_DEBUG_ROLLOUT_DATA=1 \
SEED=14317 \
ROLLOUT_SEED=14317 \
OPD_BUDGET_MASK_SEED=14317 \
...
echo "Scoring theta0 against 14B teacher..."
...
echo "Converting/scoring latest Megatron checkpoint against 14B teacher..."
...
"${PY}" tools/support_definition_robustness_20260517.py \
  --n-boot 1000 \
  --dataset 8b_diag300="${STORY_ROOT}/8b_to_1p7b_diag300/eval_latest/fixed_context_metrics.parquet" \
  --dataset heldout300_k16="${STORY_ROOT}/heldout_4b_to_1p7b_300/eval_latest/fixed_context_metrics.parquet" \
  --dataset 14b_diag300="${DIAG_DIR}/eval_latest/fixed_context_metrics.parquet"

"${PY}" - <<'PY'
from pathlib import Path
import pandas as pd

root = Path("/path/to/outputs/slime_opd/storyline_20260513/scale_context_robustness_20260517/14b_to_1p7b_diag300")
...
row = q3.iloc[0]
lines = [
    "# 14B -> 1.7B 300-context fixed-context diagnostic",
    "",
    "Source run: `/path/to/outputs/slime_opd/qwen3_1_7b_student_qwen3_14b_teacher_diag300_k16_20260517`.",
    "",
    ...
    "",
    "Interpretation: this is the paper-facing 14B teacher-scale diagnostic. Unlike the earlier 8B teacher-scale diagnostic, it is sized to be comparable to the main 4B->1.7B common-context diagnostics.",
]
