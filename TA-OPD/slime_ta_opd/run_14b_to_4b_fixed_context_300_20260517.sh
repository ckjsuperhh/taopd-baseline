...
RUN_NAME="${RUN_NAME:-qwen3_4b_student_qwen3_14b_teacher_diag300_k16_20260517}"
SAVE_DIR="${SAVE_DIR:-${OUTPUT_ROOT}/${RUN_NAME}}"
LOG_DIR="${LOG_DIR:-${OUTPUT_ROOT}/logs}"
DIAG_DIR="${DIAG_DIR:-${STORY_ROOT}/14b_to_4b_diag300}"
DRIVER_LOG="${LOG_DIR}/${RUN_NAME}_driver.log"

mkdir -p "${LOG_DIR}" "${DIAG_DIR}"
cd "${SLIME_DIR}"

echo "=== 14B -> 4B 300-context fixed-context diagnostic ==="
...
ROLLOUT_NUM_GPUS=1 \
TEACHER_PORT=13321 \
TEACHER_NCCL_PORT=23321 \
RAY_PORT=26521 \
RAY_DASHBOARD_PORT=8421 \
RAY_OBJECT_MANAGER_PORT=28721 \
RAY_NODE_MANAGER_PORT=28722 \
RAY_DASHBOARD_AGENT_LISTEN_PORT=28723 \
RAY_DASHBOARD_AGENT_GRPC_PORT=28724 \
RAY_METRICS_EXPORT_PORT=28725 \
RAY_TEMP_DIR=/tmp/slime_ray_26521 \
...
OPD_TOKEN_BANK_PAIR_ID=qwen3_14b_to_qwen3_4b \
OPD_EXACT_CMASS=1 \
OPD_EXACT_CMASS_MAX_UNION=128 \
SAVE_DEBUG_ROLLOUT_DATA=1 \
SEED=14417 \
ROLLOUT_SEED=14417 \
OPD_BUDGET_MASK_SEED=14417 \
...
root = Path("/path/to/outputs/slime_opd/storyline_20260513/scale_context_robustness_20260517/14b_to_4b_diag300")
...
row = q3.iloc[0]
lines = [
    "# 14B -> 4B 300-context fixed-context diagnostic",
    "",
    "Source run: `/path/to/outputs/slime_opd/qwen3_4b_student_qwen3_14b_teacher_diag300_k16_20260517`.",
    "",
