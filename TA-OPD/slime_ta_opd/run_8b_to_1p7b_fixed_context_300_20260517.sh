...
RUN_NAME="${RUN_NAME:-qwen3_1_7b_student_qwen3_8b_teacher_diag300_k16_20260517}"
SAVE_DIR="${SAVE_DIR:-${OUTPUT_ROOT}/${RUN_NAME}}"
LOG_DIR="${LOG_DIR:-${OUTPUT_ROOT}/logs}"
DIAG_DIR="${DIAG_DIR:-${STORY_ROOT}/8b_to_1p7b_diag300}"
DRIVER_LOG="${LOG_DIR}/${RUN_NAME}_driver.log"

mkdir -p "${LOG_DIR}" "${DIAG_DIR}"
cd "${SLIME_DIR}"

echo "=== 8B -> 1.7B 300-context fixed-context diagnostic ==="
...
ROLLOUT_NUM_GPUS=1 \
TEACHER_PORT=13251 \
TEACHER_NCCL_PORT=23251 \
RAY_PORT=26451 \
RAY_DASHBOARD_PORT=8351 \
RAY_OBJECT_MANAGER_PORT=28451 \
RAY_NODE_MANAGER_PORT=28452 \
RAY_DASHBOARD_AGENT_LISTEN_PORT=28453 \
RAY_DASHBOARD_AGENT_GRPC_PORT=28454 \
RAY_METRICS_EXPORT_PORT=28455 \
RAY_TEMP_DIR=/tmp/slime_ray_26451 \
...
root = Path("/path/to/outputs/slime_opd/storyline_20260513/scale_context_robustness_20260517/8b_to_1p7b_diag300")
...
row = q3.iloc[0]
lines = [
    "# 8B -> 1.7B 300-context fixed-context diagnostic",
    "",
    "Source run: `/path/to/outputs/slime_opd/qwen3_1_7b_student_qwen3_8b_teacher_diag300_k16_20260517`.",
    "",
    ...
    "",
    "Interpretation: this is the paper-facing teacher-scale diagnostic. Unlike the earlier eight-context smoke run, it is sized to be comparable to the main 4B->1.7B common-context diagnostics.",
]
