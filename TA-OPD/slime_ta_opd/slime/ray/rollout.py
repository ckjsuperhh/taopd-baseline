...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    def _reward_metric_value(value):
        if isinstance(value, (int, float, np.number)):
            return float(value)
        if hasattr(value, "numel") and value.numel() == 1:
            return float(value.item())
        if isinstance(value, dict):
            for key in ("reward", "score", "raw_reward", "value"):
                nested_value = value.get(key)
                metric_value = _reward_metric_value(nested_value)
    ...
    for group in interesting_sample_groups:
        reward_value = _reward_metric_value(group[0].get_reward_value(args))
        if reward_value is not None:
            interesting_rewards.append(str(round(reward_value, 1)))
