    ...
    def add_slime_arguments(parser):
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Ask the SGLang teacher to score the union of student top-k token ids, "
                    "so online Cmass becomes true teacher mass on student top-k support."
                ),
            )
            parser.add_argument(
                "--opd-exact-cmass-max-union",
                type=int,
                default=4096,
                help="Maximum per-sample union size for --opd-exact-cmass before overflow handling.",
            )
            parser.add_argument(
                "--opd-exact-cmass-overflow",
                type=str,
                choices=["fallback", "truncate", "error"],
                default="fallback",
                help=(
                    "What to do if student top-k union is larger than --opd-exact-cmass-max-union. "
            ...
            parser.add_argument(
                ...
                default="none",
                help=(
                    "Token-budget masking rule applied to response loss masks after token-bank metrics are computed. "
                    "Use none/full for standard OPD."
                ),
            )
            parser.add_argument(
                "--opd-budget-ratio",
                type=float,
                default=1.0,
                help="Fraction of valid response tokens to keep when --opd-budget-mask is enabled.",
            )
            parser.add_argument(
                "--opd-budget-gamma",
                type=float,
                default=0.5,
                help="Uncertainty budget fraction for --opd-budget-mask split_budget_ca.",
            )
            parser.add_argument(
                "--opd-budget-mask-seed",
                type=int,
                default=42,
                help="Seed for random OPD budget masking.",
            )
            parser.add_argument(
                "--opd-budget-min-keep-per-sample",
                type=int,
                default=1,
                help="Minimum selected tokens per non-empty sample when budget masking is enabled.",
            )
            parser.add_argument(
                "--opd-compat-proxy",
                type=str,
                choices=["mass", "set", "bc", "prefix_ema"],
                default="mass",
                help="Compatibility proxy used by compatibility-aware OPD budget scores.",
            )
            parser.add_argument(
                "--opd-compat-prefix-ema-alpha",
                type=float,
                default=0.9,
                help="EMA coefficient for prefix_ema compatibility proxy.",
            )
            parser.add_argument(
                "--opd-metric-normalization",
                type=str,
                choices=["batch_quantile", "batch_minmax", "none"],
                default="batch_quantile",
                help="Normalization used before TIP/compatibility budget scoring.",
            )
            parser.add_argument(
                "--opd-metric-q-low",
                type=float,
                default=0.05,
                help="Lower quantile for batch_quantile normalization.",
            )
            parser.add_argument(
                "--opd-metric-q-high",
                type=float,
                default=0.95,
                help="Upper quantile for batch_quantile normalization.",
            )
            return parser
...
def slime_validate_args(args):
    ...
    if args.use_opd:
        ...
        if args.opd_type == "megatron":
            ...
            if args.opd_budget_mask not in ("none", "full") and args.opd_topk_metrics_k <= 0:
                raise ValueError("--opd-budget-mask requires --opd-topk-metrics-k > 0.")
            if args.opd_exact_cmass and args.opd_topk_metrics_k <= 0:
                raise ValueError("--opd-exact-cmass requires --opd-topk-metrics-k > 0.")
            if args.opd_budget_ratio <= 0 or args.opd_budget_ratio > 1:
                raise ValueError("--opd-budget-ratio must be in (0, 1].")
            if not 0 <= args.opd_budget_gamma <= 1:
                raise ValueError("--opd-budget-gamma must be in [0, 1].")
            if args.opd_exact_cmass_max_union <= 0:
                raise ValueError("--opd-exact-cmass-max-union must be positive.")
            if args.opd_metric_q_low < 0 or args.opd_metric_q_high > 1 or args.opd_metric_q_low >= args.opd_metric_q_high:
                raise ValueError("--opd-metric-q-low/high must satisfy 0 <= low < high <= 1.")
    else:
        # If OPD is not enabled, opd_teacher_load should not be set
        if args.opd_teacher_load is not None:
            raise ValueError("--opd-teacher-load is set but --use-opd is not enabled. Please add --use-opd flag.")
        if args.opd_token_bank_dir is not None or args.opd_topk_metrics_k > 0 or args.opd_budget_mask not in (
            "none",
            "full",
        ):
