...
STORY_ROOT = Path("/path/to/outputs/slime_opd/analysis")
...
ARCHIVE_SCRIPT = STORY_ROOT / "collect_research_assets.py"
...
def write_summary(rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "",
        "Task: `leaderboard_math_hard` from lm-eval. This is a reproducible MATH held-out proxy on the local server, not the external Math500 table.",
        "",
...
def update_status(rows: list[dict[str, str]]) -> None:
    ...
    if rows and not procs:
        ...
        block = [
            ...
            "",
            f"Summary: `{SUMMARY_MD}`. Task is `leaderboard_math_hard`, a reproducible MATH held-out proxy on this local server. Collected models: {collected}.",
        ]
